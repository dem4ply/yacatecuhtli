...
class Test_api_item_no_autentificate( APITestCase ):
	...
	def test_retrieve_by_item( self, *args ):
		url = reverse( 'item:category_item-list', kwargs={
			'format': 'json',
			'category_pk': 1,
		} )
		# no deberia de ser valido porque no se a autentificado el usuario
	...
	def test_detail( self, *args ):
		url = reverse( 'item:item-detail', kwargs={
			'format': 'json',
			'pk': 1,
		} )
		# no deberia de ser valido porque no se a autentificado el usuario
	...
	def test_detail_by_item( self, *args ):
		url = reverse( 'item:category_item-detail', kwargs={
			'format': 'json',
			'category_pk': 1,
			'pk': 1,
		} )
	...
	def test_create_by_item( self, *args ):
		url = reverse( 'item:category_item-list', kwargs={
			'format': 'json',
			'category_pk': 1,
		} )
	...
	def test_update_by_item( self, *args ):
		url = reverse( 'item:category_item-detail', kwargs={
			'format': 'json',
			'category_pk': 1,
			'pk': 1,
...
class Test_api_item_authorize( APITestCase ):
	"""
