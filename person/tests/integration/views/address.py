...
class Test_api_address_no_autentificate( APITestCase ):
	...
	def test_retrieve_by_person( self, *args ):
		url = reverse( 'person:person_address-list', kwargs={
			'format': 'json',
			'person_pk': 1,
		} )
		# no deberia de ser valido porque no se a autentificado el usuario
	...
	def test_detail( self, *args ):
		url = reverse( 'person:address-detail', kwargs={
			'format': 'json',
			'pk': 1,
		} )
		# no deberia de ser valido porque no se a autentificado el usuario
	...
	def test_detail_by_person( self, *args ):
		url = reverse( 'person:person_address-detail', kwargs={
			'format': 'json',
			'person_pk': 1,
			'pk': 1,
		} )
	...
	def test_create_by_person( self, *args ):
		url = reverse( 'person:person_address-list', kwargs={
			'format': 'json',
			'person_pk': 1,
		} )
	...
	def test_update_by_person( self, *args ):
		url = reverse( 'person:person_address-detail', kwargs={
			'format': 'json',
			'person_pk': 1,
			'pk': 1,
...
class Test_api_address_authorize( APITestCase ):
	"""
